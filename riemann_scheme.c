...
void* calcular(void *arg) {
	...
	//    variable 'loslimit'
	float loslimit = base_length*numRectxThread*id;
  ...
  //    procesar cada hilo.
	for (int i = 0; i < numRectxThread; i++) {
		...
		partialSums[id] += (double)function(base_length*i + loslimit) * base_length;
	}
	printf("sum: %lf\n", partialSums[id]);
	pthread_exit(NULL);
}
...
int main(int argc, char** argv) {
	...
	for (int i = 0; i < MAXTHREAD; i++) {
		...
		// - calcular
		// - taskids
		pthread_create(&threads[i],NULL, calcular, (long*)taskids[i]);
	}
